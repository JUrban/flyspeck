...
#use "$flyspeck_root/formal_ineqs/arith/benchmark_cv_compute_float_taylor.hl";;
EOF
)

for marker in \
  CANDLE_CV_FLOAT_TAYLOR_OK \
  CANDLE_CV_TAYLOR_FINE_TICKS= \
  CANDLE_CV_TAYLOR_REFLECTED_TICKS= \
  CANDLE_CV_TAYLOR_EXACT_PRODUCT_TICKS= \
  CANDLE_CV_TAYLOR_REFLECTED_SUM_TICKS= \
  CANDLE_CV_TAYLOR_REPETITIONS=50 \
  CANDLE_CV_TAYLOR_BENCHMARK_OK
do
...
rg -o 'CANDLE_CV_TAYLOR_[A-Z_]+=[0-9]+' "$log_file"
printf '%s\n' 'CANDLE_CV_TAYLOR_BENCHMARK_OK'
