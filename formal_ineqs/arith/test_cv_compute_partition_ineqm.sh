...
#use "$flyspeck_root/formal_ineqs/arith/test_cv_compute_partition_ineqm.hl";;
EOF
)

if ! rg -Fq 'CANDLE_CV_PARTITION_INEQM_OK' "$log_file"; then
  tail -300 "$log_file" >&2
  exit 1
fi
