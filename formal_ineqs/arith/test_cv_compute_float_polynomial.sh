...
#use "$flyspeck_root/formal_ineqs/arith/test_cv_compute_float_polynomial.hl";;
EOF
)

if ! rg -Fq 'CANDLE_CV_FLOAT_POLYNOMIAL_OK' "$log_file"; then
  tail -420 "$log_file" >&2
  exit 1
fi
