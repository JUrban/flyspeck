...
#use "$flyspeck_root/formal_ineqs/arith/benchmark_cv_compute_float_staged_polynomial.hl";;
EOF
)

for marker in \
  CANDLE_CV_FLOAT_STAGED_POLYNOMIAL_OK \
  CANDLE_CV_STAGED_TAYLOR_LEGACY_TICKS= \
  CANDLE_CV_STAGED_TAYLOR_TICKS= \
  CANDLE_CV_STAGED_TAYLOR_REPETITIONS=10 \
  CANDLE_CV_STAGED_TAYLOR_BENCHMARK_OK
do
...
rg -o 'CANDLE_CV_STAGED_TAYLOR_[A-Z_]+=[0-9]+' "$log_file"
printf '%s\n' 'CANDLE_CV_STAGED_TAYLOR_BENCHMARK_OK'
